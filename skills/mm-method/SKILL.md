...
8. **Capture evidence as you go.** When something you read or run will become a claim in the report or a verification later, keep the exact command/source and its real output at the moment you see it - do not reconstruct it from memory afterward (that is rule 2's failure in a different disguise). `references/evidence.md` defines what a reproducible evidence item is and what is rejected.
...
3. **One command to run**, using the project's existing task runner. **No persistence, no polish, no abstractions, no tests** ("no tests" applies to the prototype only - the real implementation is still test-first in Step 4). The point is to learn fast; surface the full relevant state after every step.
...
- **Red before green, observed.** Each cycle: write one failing test at a seam, **run it, and observe it fail for the expected reason** before writing any implementation. A test that passes on first run is a bug in the test - fix the test, not the code. "Saw it fail first" is the only observable difference between TDD and back-filled tests, so it is required, not optional.
...
**Evidence standard (what counts as "observed").** A verification claim is only evidence if a fresh reader could reproduce it. For each (a)/(b) claim, hold: the **exact command or action** that produced the result (copy-pasteable; reference a secret by env-var name, never its value), and the **actual output** (the real last lines - exit code, pass/fail counts, the rendered value), not a paraphrase. "Tests pass" is not evidence; `pytest -q` -> `41 passed in 2.3s` is. Inference from absence counts only if you captured the absence (the empty result plus the query that produced it). Anything you could not run - no runtime, needs credentials, needs human eyes - is labelled unverified in the report, never dressed as observed. `references/evidence.md` is the binding definition.

**False-green defence (any non-trivial task).** A check you merely *read* has not passed - re-run it yourself and reproduce the output before claiming green; reviewing a log another step produced is not verification. And a check is only trustworthy once it has been seen to **fail**: for the Step 1 done criterion, confirm it currently passes *because the change is present* - it must have failed before the change (the observed-red test from Step 4 is exactly this) or fail now if you revert the change / inject the broken condition. A check that passes whether or not the fix is in place is a false green and gates nothing; replace it. Trivial-band work is exempt from the re-run-and-negative requirement (its one obvious check stands).
...
- The first sentence answers "what happened" or "what did you find". Detail comes after. Never include step numbers, step names, or narrative method scaffolding in the report. The only permitted method artifacts are these short verbatim lines, each owed only when its trigger was present: `INTENT:` (behavior changed), `AUTH:` (outward action taken), `PENDING:` (prescribed follow-up deliberately not taken), `TWINS:` (a defect was fixed), and the four gate lines - `GRILL:` (grill trigger present), `PROTO:` (design-risk trigger present), `SEAMS:` (a testable seam was present), `REVIEW:` (a full-loop coding change reached the Step 4 close). Formats:
  ...
  - `REVIEW: Spec <N findings> / Standards <N findings>` (two counts, never merged)  OR  `REVIEW: skipped (Small-band / trivial)`
