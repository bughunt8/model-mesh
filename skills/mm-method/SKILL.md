...
- **Mini-orient before asking.** Run one quick orientation pass (list the tree, grep the obvious spec/config) and resolve every question that pass can answer. Only what genuinely survives - a decision the environment cannot settle - becomes a question. A **fact** findable by looking is never a question; only genuine **decisions** (the user's to make) are.
...
- **A load-bearing behavioral assumption no source can settle** - you already spent the Step 2 / recall-gate lookups and docs/source do not answer it, and the behavior is *emergent* (timing, partial failure, layout).
...
4. **Capture the verdict as text, retire the code.** Write the question and the answer it settled into the report and into the revised plan, and **paste the decisive output verbatim** (the harness transcript, the interleaving that failed) into the report - that transcript is the primary source, so deleting the code loses no evidence. Then, by default, **delete the prototype** before Step 6. Only leave it in-tree if the user explicitly authorized a commit (the standing prohibition on commit/push still binds - no `AUTH:` quote, no committed branch). Either way the prototype never appears in the shipped diff.
...
2. **Run the two axes independently.** If the harness supports subagents, run them as **two subagents given only the diff, the done criterion/spec, and the standards sources - no conversation history**; that isolation is what makes the separation real, so do it whenever available. Otherwise run them as two deliberate separate passes.
...
- The first sentence answers "what happened" or "what did you find". Detail comes after. Never include step numbers, step names, or narrative method scaffolding in the report. The only permitted method artifacts are these short verbatim lines, each owed only when its trigger was present: `INTENT:` (behavior changed), `AUTH:` (outward action taken), `PENDING:` (prescribed follow-up deliberately not taken), `TWINS:` (a defect was fixed), and the four gate lines - `GRILL:` (grill trigger present), `PROTO:` (design-risk trigger present), `SEAMS:` (a testable seam was present), `REVIEW:` (the two-axis review ran). Formats:
  ...
  - `REVIEW: Spec <N findings> / Standards <N findings>` (two counts, never merged)
