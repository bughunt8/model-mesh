...
3. **A region-constrained profile may restrict its whole model set to region-reachable providers.** When a profile is scoped to a region whose network self-restricts some vendors' developer APIs, that profile drops every unreachable provider and uses **no geo-unlock proxy**, and, if the unreachable set includes the proprietary flagships, its flagship-native roles remap to the strongest region-reachable open-weight reasoner. The shipped case is the **Hong Kong-native `ultimate`** (see `docs/ROUTING.md` and `docs/EXAMPLE-MAPPING.md`); `hybrid` and `b4b` stay global. This is a documented availability remap, never a silent drop.
4. **The policy budget cap is a preference for the cheap tiers, with named exemptions, not a hard per-slot ceiling.** The cap is a repo-chosen output-price ceiling ($4.65/1M in the shipped example mapping), **not** any single vendor's price, in particular not `coder-xl`'s, whose published output price ($15/1M) is well above it. It governs the **generalist, utility, and orchestration** rungs. Three things are deliberately **exempt**: (a) the flagship **coding** tier (`coder-xl`) on the agents/categories that do the hardest implementation. In `hybrid` that is `prometheus`/`atlas` primaries plus `deep`/`ultrabrain` fallbacks, in `b4b` only the `deep` fallback; (b) the read-only architect (`oracle`); (c) each agent's `ultrawork` escape hatch. Two placeholders exceed the cap and are additionally restricted to `ultimate` only because they add no capability the exempt coder does not already provide: `flagship-open` and `div-flagship`. See `docs/EXAMPLE-MAPPING.md` for the cap note, per-model prices, and the exact over-cap slot list.
...
- Target: **any coding-agent harness.** The method is harness-agnostic; the repo ships concrete configuration examples for two harnesses: an OpenCode-based setup via **oh-my-openagent** (opencode) and the **DeepSeek Harness**. The `oh-my-openagent` / `opencode` identifiers are kept intentionally because the OpenCode example config *must* contain them to load (see README "What's genericized vs kept"); the DeepSeek Harness example likewise keeps its own required `settings.yaml` keys. These are framework identifiers, not model choices.
