...
> **Two mappings live in this file.** The provider/model tables immediately below (with the `apiyi` proxy row and the GPT / Claude / Grok model IDs) are the mapping for the **global** profiles, `hybrid` and `b4b`. The `ultimate` profile now uses a **different, Hong Kong-native mapping**, see [The Hong Kong-native `ultimate` mapping](#the-hong-kong-native-ultimate-mapping) below. Reading the top tables as if they applied to `ultimate` would be wrong: `ultimate` no longer uses `apiyi`, GPT, Claude, or Grok at all.

## Provider placeholders (hybrid / b4b, global)
...
## Model placeholders (hybrid / b4b, global)
...
| `ProviderB/creative-mid` | `zai-coding-plan/glm-5.3` | Creative generalist (distinct from `gen-pro` and `retrieval-mid`) |
| `ProviderB/budget-low` | `opencode/deepseek-v4-flash-free` | Cheapest general |
| `ProviderB/flagship-open` | `opencode/qwen3.8-max` | GA frontier-class open flagship; used in the HK-native `ultimate` (cost > cap, so kept out of the metered profiles) |
| `ProviderF/vision-xl` | `minimax/MiniMax-M3` | Native multimodal + long context |
| `ProviderG/div-flagship` | `xai/grok-4.6` | Grok diversification rung for the **pre-HK global** ultimate. Dropped from the current HK-native `ultimate` (xAI has no HK developer API) and never used in the metered profiles (cost > cap). Not referenced by any current profile. |

## The Hong Kong-native `ultimate` mapping

The `ultimate` profile is now **Hong Kong-native**. It is mapped only to models whose **developer API is natively reachable from Hong Kong**, and it uses **no geo-unlock proxy** (there is **no `apiyi` row** in this mapping). This is a deliberate, region-forced remap of `ultimate` only. `hybrid` and `b4b` are **unchanged and remain global** (they still use the `apiyi` / GPT / Claude / Grok mapping in the tables above).

### Why these vendors are excluded from `ultimate`

The US frontier vendors self-restrict developer-API access from Hong Kong, so their coding-relevant APIs cannot be consumed natively there:

- **OpenAI (GPT).** Hong Kong is not on OpenAI's supported-countries list, and OpenAI curbed API access from Hong Kong ([OpenAI supported countries](https://developers.openai.com/api/docs/supported-countries); [SCMP: OpenAI curbs API access](https://www.scmp.com/tech/big-tech/article/3268233/microsoft-maintains-ai-services-hong-kong-openai-curbs-api-access-china)).
...
- **Google (Gemini).** The **native Gemini API / Google AI Studio still refuse Hong Kong** ([Gemini API available regions](https://ai.google.dev/gemini-api/docs/available-regions)). A **consumer Gemini web app launched in Hong Kong (Mar 2026)**, but that does **not** grant developer-API access, the API remains blocked ([Gemini HK: app live, API blocked](https://10beasts.net/gemini-hong-kong-api-blocked-vpn/)). The only compliant Google *developer* route is enterprise **Vertex AI in the `asia-east2` (Hong Kong) region** ([Vertex AI locations](https://docs.cloud.google.com/gemini-enterprise-agent-platform/resources/locations)). Because a coding harness consumes a raw model API, Gemini is treated here as **not natively available**.
- **xAI (Grok).** Excluded on the same US-vendor availability basis ([HK AI wall overview](https://hongkongaipodcast.com/blog/hk-ai-wall)).

Since `ultimate` drops every model behind these gateways, its flagship-native roles cannot be filled by a GPT/Claude family model. Under the HK constraint they map instead to the strongest HK-reachable **open-weight** reasoner. This is an **availability-forced family remap, documented here**, not a silent capability downgrade.

### Provider placeholders (ultimate, Hong Kong-native)

| Placeholder | Real provider | HK-reachable |
|---|---|---|
| `ProviderC` | `deepseek` (open-weight reasoning) | ✓ |
| `ProviderD` | `zai-coding-plan` (Z.ai communicator-class) | ✓ |
| `ProviderE` | `moonshotai` (coding-specialist family) | ✓ |
| `ProviderF` | `minimax` (multimodal / long-context) | ✓ |
| `ProviderB` | `opencode` (general gateway; used only for HK-reachable models) | ✓ |

There is intentionally **no `ProviderA` (apiyi proxy)**, **no `reasoner-xl` (Claude)**, and **no `div-flagship` / `ProviderG` (Grok)** in the `ultimate` mapping.

### Model placeholders (ultimate, Hong Kong-native)

| Placeholder | Real model ID | Role under HK constraint |
|---|---|---|
| `ProviderC/open-reason-xl` | `deepseek/deepseek-v4-pro` | Flagship-native roles `hephaestus` / `oracle` / `momus` (reasoning `max`); the strongest HK-reachable reasoner fills the seat a GPT/Claude flagship would hold in a global profile |
| `ProviderC/open-reason-lite` | `deepseek/deepseek-v4-flash` | Fast open-weight reasoner / utility |
| `ProviderE/coder-xl` | `moonshotai/kimi-k3` | `atlas` / `prometheus` primaries; `hephaestus` fallback + `ultrawork` escape hatch |
| `ProviderE/coder-mid` | `moonshotai/kimi-k2.7-code` | Coding workhorse fallback |
| `ProviderB/coder-swarm` | `opencode/kimi-k2.6` | Long-context / swarm coding fallback |
| `ProviderD/comm-xl` | `zai-coding-plan/glm-5.3` | Orchestration `sisyphus` / `metis` |
| `ProviderD/comm-lite` | `zai-coding-plan/glm-5.3-flash` | Cheap communicator |
| `ProviderF/vision-xl` | `minimax/MiniMax-M3` | `multimodal-looker` (native vision + long context) |
| `ProviderB/flagship-open` | `opencode/qwen3.8-max` | GA open flagship fallback |
| `ProviderB/gen-pro` | `opencode/qwen3.7-plus` | Planning / writing / visual generalist |
| `ProviderB/gen-flash` | `opencode/qwen3.5-plus` | Fast utility loop steps |
| `ProviderB/retrieval-mid` | `opencode/qwen3.6-plus` | Retrieval / librarian |
| `ProviderB/creative-mid` | `zai-coding-plan/glm-5.3` | Creative generalist |
| `ProviderB/open-coder` | `opencode/mimo-v2.5-free` | Token-efficient open-weight coder (MiMo) |
| `ProviderB/budget-low` | `opencode/deepseek-v4-flash-free` | Cheapest general |

The full HK-reachable model set `ultimate` routes to: **DeepSeek V4-Pro / V4-Flash**, **Kimi K3 / K2.7-code / K2.6** (Moonshot), **GLM-5.3 / GLM-5.3-Flash** (Z.ai), **Qwen3.8-max / 3.7 / 3.6 / 3.5-plus** (Alibaba), **MiniMax-M3**, and **MiMo**. Every owner here has a developer API natively reachable from Hong Kong; no US-owned model is routed, not even through a US-hosted gateway. Concrete `ultimate` chains (primaries, fallbacks, `ultrawork`) are in [`profiles/ultimate.example.json`](../profiles/ultimate.example.json).
...
- `comm-lite` / `vision-lite` (`glm-5.3-flash`): [GLM-5.3-Flash](https://docs.z.ai/release-notes/new-released) (released 2026-08-26) replaces the stale `glm-5-turbo` (no longer on Z.ai's current rate card) as the cheap communicator, and is added as a cheap **multimodal fallback** on `multimodal-looker` (it has [native vision](https://docs.z.ai/release-notes/new-released); 320B/18B MoE, hybrid attention). On the one **independent, ground-truth** vision benchmark that has run it, [Roboflow Vision Evals](https://playground.roboflow.com/models/z-ai/glm-5-3-flash), which scores every model against the same real-world samples, it lands mid-pack at **66.3% overall (#23 of 34)**, strong on OCR (90.6%) and identification (84.4%) but weak on object detection (33.1%), roughly level with Claude Sonnet 5 (66.4%). Its vendor-headline MVBench 77.8 / MMVU 80.5 figures are [self-reported and unverified](https://llm-stats.com/benchmarks/mvbench) (the MVBench leaderboard notes 0 verified vs 18 self-reported results). Given a mid-pack independent score at rock-bottom cost, its placement ahead of a flagship-lite vision rung in `b4b` is a **cost-first** ordering, not a capability claim. Priced [$0.15 in / $0.50 out per 1M list](https://docs.z.ai/guides/overview/pricing) (promo $0.075/$0.25 until 2026-09-09). Both placeholders intentionally resolve to the same model, a dual-role mapping (like `coder-swarm`'s two-provider entries), not a duplicate.
...
- `div-flagship` (`xai/grok-4.6`) was a diversification fallback on `atlas` and `prometheus` in the **pre-HK global** `ultimate`. It is **removed from the current Hong Kong-native `ultimate`** because xAI has no HK-native developer API, and it was never in the metered profiles (its $6 output exceeds the $4.65/1M policy budget cap). It is retained here only as a historical placeholder definition and is **not referenced by any current profile**. For the record: Grok trailed the stack's primaries on role-relevant benchmarks ([Terminal-Bench 2.1 83.3 vs kimi-k3 88.3 / deepseek-0813 87.9 / glm-5.3 88.2](https://www.tldevtech.com/grok-45-benchmarked-where-it-actually-wins)) but added an independent 5th vendor and was token-efficient ([~16k output tok/task](https://hokai.io/hub/models/grok-4.5)); priced [$2/$6 per 1M](https://benchlm.ai/xai/api-pricing).
...
- `creative-mid` was remapped from `qwen3.7-plus` to `glm-5.3` so it stays distinct from **both** `gen-pro` (`qwen3.7-plus`) and `retrieval-mid` (`qwen3.6-plus`). An intermediate fix to `qwen3.6-plus` would have merely traded the `gen-pro` collision for a `retrieval-mid` one. An earlier draft used `muse-spark-1.2`, but that is a Meta model with no HK-native developer API (US-only, reachable only through a US-hosted relay), which breaks the profile's no-US-owned-model rule; `glm-5.3` (Z.ai) is a genuinely distinct creative generalist that is natively HK-reachable.
...
| `reasoner-xl` = claude-opus-5 (uncapped tier) | **none** (dropped from HK-native `ultimate`; never in metered) | Anthropic has no HK developer API; not referenced by any current profile |
| `flagship-open` = qwen3.8-max ($6) | `ultimate` only | Adds no capability the exempt coder lacks; kept out of metered profiles |
| `div-flagship` = grok-4.6 ($6) | **none** (dropped from HK-native `ultimate`; never in metered) | xAI has no HK developer API; not referenced by any current profile |
