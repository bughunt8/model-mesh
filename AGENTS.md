...
> Portable, **harness-agnostic** instructions for any coding agent or harness (OpenCode, the DeepSeek Harness, Cursor, aider, or a raw system prompt). The method is not tied to one harness; the repo ships concrete configuration examples for two of them, **OpenCode** (via oh-my-openagent) and the **DeepSeek Harness**, while these instructions stay portable. Two parts: **(A) the routing setup**, how to configure multi-model roles, and **(B) the loop**, how to do any task well. Part B is identical in spirit to the `mm-method` SKILL; follow it literally. The steps structure your work, never your output: do not narrate step numbers in anything the user reads.

---

# Part A - Routing setup (LLM runbook)
...
6. **Emit the deployable file.** Wrap the profile in the `[opencode]` structure shown in `omo.jsonc`. Enforce the schema:
   - Agents use `model` (singular) + `fallback_models` (array). **Never `models` on an agent.**
   - `ultrawork` uses a singular `model` (+ optional `reasoning`). No `models`, no fallback list.
   - Categories use a `models` array (primary first, then fallbacks in order).
   - Use `reasoning` (not the deprecated `variant`). Strip any BOM; JSONC comments and trailing commas are fine in the file but must parse.
7. **Validate.** Run `bunx oh-my-openagent doctor` and fix every reported key until clean.
8. **Report outcome-first** (Part B, Step 6). List any placeholder you could not confidently map, and any model whose family fit you were unsure about.

**Config fit gate.** If the user's real models do not cover a role's family, say so and route: propose the closest same-family option, or hand back for a decision. Never silently drop a role onto a wrong-family model.

---

# Part B - The loop (any task)
